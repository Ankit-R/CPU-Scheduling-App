...
public class MainActivity extends Activity {
	...
	int n;
	String str;
	String title = "PRIORITY SCHEDULING";
		...
		btn_main.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View arg0) {
	            ...
	            for (int i = 0; i < n; i++) {
	                ...
	                p_value[i] = new EditText(getBaseContext());
	                p_value[i].setGravity(Gravity.CENTER);
	                p_value[i].setTextColor(Color.BLACK);     
	                p_value[i].setInputType(InputType.TYPE_CLASS_NUMBER);
	                color = Color.parseColor("#000000");
            	    drawable = p_value[i].getBackground();
            	    drawable.setColorFilter(color, PorterDuff.Mode.SRC_ATOP);
            	    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN) {
            	        p_value[i].setBackground(drawable);
            	    } 
            	    else {
            	        p_value[i].setCompoundDrawables(null, null, drawable, null);
            	    }
	                tbrow.addView(p_value[i]);
					...
					compute.setOnClickListener(new OnClickListener() {
						...
						public void onClick(View arg0) {
							...
							for(int i=0; i<n; i++){
								String str = p_value[i].getText().toString();
								
								 if(TextUtils.isEmpty(str)) {
								    p_value[i].setError("Fill this field");
								    return;
								 } 
								}
							...
							int i,j,temp,time,time1,a,b,T=0;
							...
							for(i=0;i<n;i++){
								...
								PRIORITY[i] = Integer.parseInt(p_value[i].getText().toString());
								process[i]=i+1;
								if(ET[i]==0)f[i]=1;
								else
								f[i]=0;
							...
							int x1[]=new int[time1];       // ankit start declare this all   
							int y1[]=new int[time1];
							int z1[]=new int[time1+1];     // note this time1+1
							int x2[]=new int[time1];
							int y2[]=new int[time1];
							int z2[]=new int[time1];
							int tp=0,tp1=0;
							z1[time1]=999;         //ankit end 
					        ...
					        for(i=0;i<a;i++)
							{
								if(f[i]==0)
								{
						           ...
						           // show_gantt.setText(Html.fromHtml("______<br>|&nbsp;&nbsp;P"+x+"&nbsp;&nbsp;&nbsp;&nbsp;|"+"<br>"+"______"+""));
					                ...
					                x1[time]=x;                 // ankit new part
					        		y1[time]=y;                   // ankit new part
					        		z1[time]=z;   
					                
					                
					                
					                  //System.out.println(y+" P"+x+" "+z);	// ANKIT HERE U CAN DO THAT GUI
							time++;
							}//end of while	
							
							tp=-1; tp1=0;         //DECLARATIONS
							for(i=0;i<time1;i++)        //DECLARATIONS ANKIT code start
							{
							...
							tp++;              //DECLARATIONS  ANKIT THIS ENDS code
							...
							/*System.out.println("tp end "+tp);*/    
							for(i=0;i<tp;i++) {              // ankit use this i=0;to i<=tp 
							/*System.out.println(y2[i]+" P"+x2[i]+" "+z2[i]);*/    //note <= 
							                                					// use x2[] y2[] z2[] at the place x y z
